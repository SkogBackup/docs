...
type: note
permalink: skogai/skills/skogai-developing-for-claude-code/references/troubleshooting
---
...
1. **Test explicitly** Ask for a task that exactly matches the description:
...
1. Verify marketplace is added:
   ...
   ```
1. Check marketplace.json format:
   ```json
   {
     "name": "my-dev",
     "plugins": [{
...
1. Uninstall → modify → reinstall → restart:
   ...
   # Restart Claude Code
   ```
1. For hook/MCP changes, restart is mandatory
1. For skill/command content changes, sometimes works without restart (but safer to restart)
...
1. **Check all paths use variables**
...
1. **Check example plugins** in this repo and `~/.claude/plugins/`
1. **Simplify** - Remove components until it works, then add back one at a time
