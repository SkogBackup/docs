...
2. **Configure dual remotes**: Set up both `origin` (pointing to our fork) and `upstream` (pointing to the original repository)
3. **Synchronize regularly**: Keep master and develop branches in sync with upstream
4. **Use feature branches**: Develop custom modifications in feature branches
...
2. Finalize, test, and commit any release adjustments
3. Finish release: `git flow release finish x.x.x`
   - Merges to master
   - Tags with version
...
2. Identify conflict source: `git status`
3. Resolve conflicts in editor
4. Mark as resolved: `git add [conflicted-files]`
5. Continue the operation (merge, rebase, etc.)
6. Push changes when complete
...
2. Clone your fork: `git clone https://github.com/SkogAI/repository.git`
3. Add upstream remote: `git remote add upstream https://github.com/original/repository.git`
