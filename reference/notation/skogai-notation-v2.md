...
2. **Modal**: Necessity, possibility, actuality
3. **Temporal**: Duration, sequence, event
4. **Spatial**: Location, extension, boundary  
5. **Social**: Agency, interaction, capability
6. **Existential**: Being, becoming, difference

### Usage Protocols
1. **Symbol Precedence**: Follow dimensional hierarchy (0D → 1D → 2D)
2. **Type Safety**: Ensure dimensional compatibility
3. **Consistency**: Verify no circular `$` definitions
4. **Security**: Honor `.` operator constraints
5. **Effects**: Track `@` operator side-effects
