...
type: note
permalink: ontology/identity-and-uniqueness-analysis-1
---
...
- `"$parent": "$id|_"` - parent as choice between identity or existence
- `"eid": "$entity.id*$entity.gen"`

## In Relational Contexts

- `[$id=$id]` - identical identity is exactly itself
- `{$id1@$id2}` - identity acting on itself must change
