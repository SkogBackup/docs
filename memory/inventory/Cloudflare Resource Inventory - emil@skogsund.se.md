...
*Inventoried: August 14, 2025*

## Account Overview

**Primary Account**: emil@skogsund.se (created June 6, 2024)
**Alternative Account**: Niklas@aldervall.se (created April 12, 2023)
...
2. **Database schemas**: What tables/data are in each D1 database?
3. **Bucket contents**: What files are stored in each R2 bucket?
4. **KV data types**: What kind of data is in each namespace?
5. **Worker functions**: What does each Worker actually do?
6. **Dependencies**: Which Workers connect to which databases/storage?
7. **AutoRAG content**: What knowledge is indexed in the skogai AutoRAG?
8. **Second account**: Should we also inventory the Niklas@aldervall.se account?
...
**Account Status**: Primary development/testing account for Emil Skogsund
**Current Usage**: Experimental and testing projects - no active production systems
**Resource Status**: All 15 Workers and associated resources are from testing/exploration phases
**Future Direction**: Looking to build permanent, production-ready systems with MCP collaboration
