...
---
title: skogparse-vision
type: note
permalink: skogai/vision/skogparse-vision
---

# SkogParse Vision

SkogParse is a powerful text parsing utility that processes `[@command:parameter]` style tags, integrating seamlessly with the broader SkogAI ecosystem through consistent design patterns.

## Core Functionality

SkogParse identifies and processes inline commands in text:
- `[@command:parameter]` format for standard commands
- `[@@command:parameter]` for recursive processing
- Parameters can be colon-separated for multiple arguments

## Command Types

The system supports multiple command behaviors:

1. **Print Commands**: Replace tag with script output
   - Example: `[@date:now]` → `2025-04-30 13:25:26`

2. **Script Commands**: Execute without replacement
   - Example: `[@log:event]` (writes to log, doesn't change text)

3. **Shell Commands**: Execute shell operations
   - Example: `[@shell:ls -la]`

4. **Background Commands**: Run asynchronously
   - Example: `[@dot:"question"]` (processes in background)

5. **Recursive Commands**: Parse output again
   - Example: `[@@template:file]` (processes any commands in the template)

## Integration with SkogAI Ecosystem

SkogParse follows the consistent patterns established across SkogAI tools:

### Script Discovery

Commands are automatically discovered from the `./scripts` directory:
- Top-level scripts become commands (e.g., `./scripts/date.sh` → `[@date:parameter]`)
- Subdirectories create namespaces (e.g., `./scripts/utils/format.sh` → `[@utils/format:parameter]`)
- No configuration needed - just add scripts

### Integration with skogcli

SkogParse leverages existing script management:
- Script management via `skogcli misc` commands
- AI-generated scripts using `skogcli misc generate`
- Common execution patterns shared across tools

### Embedding SkogAI Commands

SkogParse enables embedding any SkogAI ecosystem command in text:
- `[@chat/send/goose:Hi goose!]` → Sends a message to Goose
- `[@context/create:sc-docs]` → Creates documentation context
- `[@parse/chat:history.latest]` → Parses chat history

## Plugin Architecture

The system supports plugin-like extension through directory structure:
- Create subdirectories for logical grouping
- Commands are automatically namespaced by location
- No registration required - structure is discovery

## Recursive Capabilities

SkogParse can process nested configurations through recursive parsing:
```
[@config:chat-history] 
  └── List of [@config:message]
       ├── [@config:type]
       └── [@config:agent]
            └── [@config:name]
                 └── [@config:string]
...
The core implementation follows these principles:
- Scripts in `./scripts` directory map directly to commands
- Universal script execution delegates behavior to individual scripts
- Special case for `[@@]` recursive processing
- Directory structure serves as the command registry

This enables maximum flexibility with minimum complexity, achieved through consistent patterns rather than rigid frameworks.

---
---
---

Above is the knowledge guide for skogai-memory as well as a memory called skogparse-vision.md

Please return your ratings and what you see as good and bad about the memory skogparse-vision.md and how it follows the skogai-memory knowledge guide.