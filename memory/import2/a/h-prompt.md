...
---
title: SkogAI Syntax Guide
type: note
permalink: skogai/skog-ai-syntax-guide
---

# SkogAI Syntax Guide

This guide documents the standard syntax patterns used across SkogAI projects, with special attention to notation like `[]`, `[[]]`, and other semantic markers.

## Core Syntax Elements

### Categories `[category]`

Square brackets denote categories or types that classify information:

```markdown
- [idea] This represents a new concept or proposal
- [decision] This indicates a choice that was made
- [question] This marks an inquiry or uncertainty
- [fact] This represents verified information
- [requirement] This indicates something necessary
- [technique] This describes a method or approach
- [recipe] This outlines a process or procedure
- [preference] This notes a subjective choice
```

Categories provide semantic context to observations, making them more meaningful within the knowledge graph.

### Wikilinks `[[Entity Name]]`

Double square brackets create references to other entities in the knowledge base:

```markdown
- relates_to [[Project Overview]]
- implements [[Core Design Principles]]
```

Wikilinks establish connections between different entities, creating the foundation of the knowledge graph. They can reference:

- Existing entities by their exact titles
- Non-existent entities (forward references) that will be automatically linked when created later

### Relation Types `relation_type [[Entity]]`

A relation type followed by a wikilink defines a specific relationship:

```markdown
- depends_on [[Authentication System]]
- extends [[Base Component]]
- part_of [[Larger Framework]]
```
...
- `pairs_with`: Complementary connection
- `inspired_by`: Influence relationship
- `originated_from`: Source relationship

### Tags `#tag`

Hashtags provide additional classification and filtering capabilities:

```markdown
- [technique] Use incremental backups for efficiency #backup #performance
```

Tags allow for cross-cutting categorization that complements the hierarchical structure of folders and relations.

### Context Parentheses `(additional context)`

Parentheses after observations or relations provide supplementary information:

```markdown
- [decision] Selected PostgreSQL for data storage (better performance with our workload)
- requires [[Authentication System]] (for secure user management)
```

This additional context helps explain the reasoning or specifics without cluttering the main knowledge graph structure.

## Memory URIs `memory://`

The `memory://` URI scheme provides a standardized way to reference entities within the knowledge graph:
...
These URIs enable precise navigation of the knowledge graph relationships.

## Syntax Usage Across SkogAI Projects

While the syntax originated in the SkogAI-Memory project, it is used consistently across the SkogAI ecosystem:

1. **SkogAI-Memory**: Full implementation of all syntax elements for knowledge management
2. **SkogAI Command Line**: Uses syntax for filtering and retrieving information
3. **SkogAI API**: Accepts and returns data structured according to these patterns
4. **SkogAI Integrations**: Translates between this syntax and external system formats

## Best Practices

### Semantic Density

Include multiple observations with appropriate categories in each note to increase semantic density.

### Precise References

Always try to use exact titles when creating wikilinks to existing entities.

### Deliberate Relations

Choose specific relation types that accurately convey the nature of connections.

### Balanced Context

Include enough context parentheses to clarify but not overwhelm the core information.

### Consistent Categories

Use the same category types consistently across related content.
...
## Overview
Secure user authentication component for the platform.

## Observations
- [requirement] Must support OAuth 2.0 protocol #security #standard
- [decision] Using JWT for stateless authentication (reduces database load)
- [technique] Refresh token rotation for enhanced security #security

## Relations
- part_of [[User Management]]
...
```markdown
# API Design Meeting Notes

## Key Points
Discussion of REST vs GraphQL for the new service endpoints.

## Observations
- [question] How will this handle complex nested queries? #performance
- [decision] We'll use GraphQL for the product catalog (better flexibility)
- [fact] Current clients require REST compatibility #compatibility

## Relations
- relates_to [[Backend Architecture]]
- precedes [[API Implkiementation Phase]]
- resolves [[API Technology Selection]]
```

This standardized syntax enables consistent knowledge representation across the SkogAI ecosystem, facilitating meaningful connections and rich semantic context.

---
---
---

Above is the knowledge guide for skogai-memory as well as a memory called h.md

Please return your ratings and what you see as good and bad about the memory h.md and how it follows the skogai-memory knowledge guide.