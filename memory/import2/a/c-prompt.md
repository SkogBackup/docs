...
---
title: SkogParse Command Syntax
type: note
permalink: skog-ai/tools/skog-parse-command-syntax
---

# SkogParse Command Syntax Guide

SkogParse is a text parsing utility that allows embedding commands within text and executing them.

## Syntax Overview

SkogParse supports two main command formats:

1. **Standalone commands** on their own line:
   ```
   @command:payload
   ```

2. **Inline commands** embedded within text:
   ```
   Some text [@command:payload] more text
   ```

## How It Works

When you run `skogparse parse "your text with [@commands]"`, the following happens:

1. The parser identifies all command patterns in the text
2. For each command:
   - It extracts the command name and payload
   - It looks for a handler for that command name
   - It executes the handler with the given payload
   - It replaces the command pattern with the handler's output

## Command Resolution

Commands are resolved in this order:

1. First, it checks for a shell script at `./scripts/{command}.sh`
   - If found, it executes the script with the payload as an argument
   - The script output becomes the command result

2. If no script exists, it looks for a registered handler function
   - Handlers can be registered via Python API or CLI
   - The handler receives the payload and returns a result

## Example

Input:
```
Hello [@greet:John] how are you today? [@date:] shows the current date.
```

Output (after processing):
```
Hello Good morning, John! how are you today? April 30, 2025 shows the current date.
```

## Command Implementation

You can implement commands in two ways:

1. **Shell Scripts**: Create `.sh` files in the `./scripts/` directory
   ```bash
   # scripts/greet.sh
   #!/bin/bash
   echo "Good morning, $1!"
...
2. **Python Handlers**: Register functions in code
   ```python
   from skogparse import register_handler
   
   register_handler("greet", lambda name: f"Good morning, {name}!")
   ```

## Nesting and Payload Processing

- Commands can be nested in theory, but by default nested commands are not processed
- JSON payloads are automatically parsed into Python objects for handlers

## Command Output

The command output completely replaces the command pattern in the text.

---
---
---

Above is the knowledge guide for skogai-memory as well as a memory called c.md

Please return your ratings and what you see as good and bad about the memory c.md and how it follows the skogai-memory knowledge guide.