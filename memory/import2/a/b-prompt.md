...
---
title: SkogParse Command Examples
type: note
permalink: skog-ai/tools/skog-parse-command-examples
---

# SkogParse Command Examples

This note contains practical examples of using SkogParse commands in different contexts.
...
Today is [@date:]
```
Output: `Today is April 30, 2025`

### Math Calculations

```
The result is [@calc:2+3*4]
```
Output: `The result is 14`

## Script-Based Commands

SkogParse will automatically execute shell scripts in the `./scripts/` directory:
...
# scripts/json.sh
#!/bin/bash
echo "Received JSON with keys: $(echo $1 | jq 'keys[]')"
```
...
```
Output: `Received JSON with keys: name age`

## Combining Multiple Commands

SkogParse can process multiple commands in a single text:

```
Hello [@greet:John]! Today is [@date:] and your calculation result is [@calc:10/2].
```

Output:
```
Hello Hello, John!! Today is April 30, 2025 and your calculation result is 5.
```
...
```
Dear [@customer:{"id":123,"name":"Jane"}],

Thank you for your order #[@order:ORD-12345].

Your shipment will arrive on [@shipping:{"date":"2025-05-05"}].
...
```
I'll help you with that task. Let me check [@debug:task-123] first.
Based on my analysis [@analyze:parameters], I recommend the following approach.
```
...
While nested commands aren't processed by default, you can create the effect of command chaining by using separate commands:

```
The result of processing is [@process:[@data:raw]]
```

This won't work as expected. Instead, use:

```
The raw data is [@data:raw].
The processed result is [@process:$DATA_RESULT].
```

Where the second command uses a variable or storage mechanism to access the result of the first command.

---
---
---

Above is the knowledge guide for skogai-memory as well as a memory called b.md

Please return your ratings and what you see as good and bad about the memory b.md and how it follows the skogai-memory knowledge guide.