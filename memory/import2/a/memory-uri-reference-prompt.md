...
# Memory URI Reference Guide

This guide provides comprehensive information about the Memory URI system used in SkogAI Memory to reference and navigate knowledge.

## What Are Memory URIs?

Memory URIs (Uniform Resource Identifiers) provide a standardized way to reference specific content in your knowledge base, functioning like an addressing system for accessing and connecting information.
...
- `memory://` - Protocol identifier that marks this as a memory system reference
- `[resource-type]` - The type of content being referenced
- `[identifier]` - The specific item's unique identifier, permalink, or path

## Resource Types

### `note`

References a note by its title or permalink.

```
memory://note/my-note-title
```

The system will locate the note regardless of its location in the folder structure.

### `entity`

References content by its full path in the folder structure.

```
memory://entity/folder/subfolder/document-name
```

This provides precise targeting of content in a specific location.

### `conversation`

References chat history or discussion threads.
...
Useful for referencing specific discussions or continuations.

### `content`

References raw file content by path.

```
memory://content/path/to/file.md
```

Used for accessing the actual file content rather than processed document.

## Advanced URI Features

### Query Parameters

Some URIs support additional parameters to control how content is retrieved:

```
memory://entity/projects/my-project?depth=2[@file:memory-uri-reference.md]timeframe=7d
```

Common parameters include:
- `depth`: How many levels of related content to include
- `timeframe`: Time restriction for content (e.g., "7d" for past week)
...
```
memory://collection/tags/productivity
memory://collection/category/technique
```

Returns all content with matching tags or categories.

## Using URIs in Practice

### In Relations

```markdown
## relations
- relates_to [[document-name]] (creates direct link)
```

The system automatically converts the bracketed reference to the appropriate URI.
...
```markdown
[See this document](memory://note/document-name)
```

Creates clickable links that resolve within the knowledge system.

## URI Quick Reference Table

| Pattern | Purpose | Example |
|---------|---------|---------|
| `memory://note/title` | Access by title | `memory://note/knowledge-management` |
| `memory://entity/path/to/note` | Access by path | `memory://entity/projects/project-a/overview` |
| `memory://entity/conversation/id` | Reference discussion | `memory://entity/conversation/2023-04-15-1234` |
| `memory://content/path/to/file` | Reference raw content | `memory://content/resources/diagram.md` |
| `memory://collection/tags/tag` | Access by tag | `memory://collection/tags/productivity` |
...
- Check exact spelling of resource types and identifiers
- Verify the referenced resource actually exists
- Try using search to find the correct identifier
- Check if special characters need URL encoding

### Ambiguous References

If multiple resources could match a URI:
- Use more specific resource types
- Use full paths instead of just titles
- Include more of the folder hierarchy

### Performance Issues

When working with large result sets:
- Add limit parameters to restrict results
- Use more specific URI patterns
- Narrow timeframes for temporal queries

## observations
- [fact] Memory URIs follow a standard protocol format for consistency #standards #format
- [principle] Uniform addressing enables reliable reference across the knowledge base #reference #reliability
- [technique] Different resource types allow precise targeting of specific content #specificity #navigation
- [fact] URI parameters provide control over how related content is included #control #context
- [requirement] URI references must be properly formatted to resolve correctly #syntax #correctness

## relations
- part_of [[memory-system]] (addressing component of the knowledge system)
- relates_to [[knowledge-graph-navigation]] (enables traversal of knowledge connections)
- implements [[reference-standards]] (standardized addressing approach)

---
---
---

Above is the knowledge guide for skogai-memory as well as a memory called memory-uri-reference.md

Please return your ratings and what you see as good and bad about the memory memory-uri-reference.md and how it follows the skogai-memory knowledge guide.