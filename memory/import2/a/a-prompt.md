...
---
title: Using SkogParse in SkogAI
type: note
permalink: skog-ai/integration/using-skog-parse-in-skog-ai
---

# Using SkogParse in SkogAI

SkogParse is a powerful text parsing and command execution utility integrated with SkogAI. This guide explains how to use SkogParse's inline command functionality within the SkogAI ecosystem.

## Basic Usage

You can embed SkogParse commands in any text that gets processed by the SkogParse system. The most common syntax is:

```
[@command:parameter]
```

When this text is processed, the command is executed and replaced with its output.

## Example with Command Line

```bash
skogparse parse "Hello there! [@greet:John]"
```

Output:
```
Text content:
Hello there! Hello, John!
```

## Available Commands in SkogAI

The following commands are currently available in the SkogAI environment:

- `[@analyze:parameter]` - Analyzes the parameter
- `[@calc:expression]` - Evaluates the mathematical expression
- `[@date:format?]` - Shows the current date/time (optional format)
- `[@debug:parameter]` - Shows debug information
- `[@dot:message]` - Sends a message to Dot (SkogAI assistant)
- `[@goose:message]` - Sends a message to Goose (SkogAI assistant)
- `[@greet:name]` - Greets the provided name
- `[@hello:name]` - Simple hello message
- `[@json:json_data]` - Processes JSON data
- `[@parse:text]` - Parses the provided text
- `[@scripts:]` - Lists available scripts
- `[@skogix:message]` - Sends a message to Skogix

## Integration with AI Assistants

One of the key features of SkogParse in SkogAI is the ability to communicate with AI assistants directly through inline commands.

For example:
```
I need some help with this problem: [@dot: Can you explain how to solve this equation: 2x + 5 = 15?]
```

When processed, this will:
1. Send the message to the Dot assistant
2. Get Dot's response
3. Replace the command with Dot's answer

## Creating Custom Commands

You can create your own commands by adding shell scripts to the `scripts/` directory:

1. Create a file named `your_command.sh` in the scripts directory
2. Make it executable (`chmod +x your_command.sh`)
3. Write your script to process the input and produce output
...
While nested commands aren't directly supported, you can achieve similar effects by processing text sequentially:

```bash
echo "First result: [@calc:2+3]" > temp.txt
skogparse parse "$(cat temp.txt) and second: [@calc:4*5]"
```

### Using Commands in Notes

Commands can be particularly useful in notes and documentation:
...
Current status is: [@status:project-123]
...
- Test complex command chains before using them in production

---
---
---

Above is the knowledge guide for skogai-memory as well as a memory called a.md

Please return your ratings and what you see as good and bad about the memory a.md and how it follows the skogai-memory knowledge guide.