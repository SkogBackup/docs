...
---
title: skogai-memory-quick-reference
type: note
permalink: skogai/memory/skogai-memory-quick-reference
---

# SkogAI-Memory Quick Reference

This document provides concise, easily referenceable information about SkogAI-Memory commands, patterns, and usage.

## Key Concepts

- **Knowledge Graph**: A network of connected information pieces
- **Entities**: Notes, files, and other objects in your knowledge base
- **Relations**: Connections between entities
- **Observations**: Categorized facts, decisions, and other information
- **Permalinks**: Unique identifiers for entities
...
This convention helps maintain consistency across the knowledge base and simplifies reference via memory:// URLs.
...
```markdown
- [fact] This is a factual observation #tag
- [decision] This records a decision that was made #category
- [question] This notes an outstanding question #research
- [idea] This captures a new concept or proposal #innovation
- [requirement] This defines something necessary #specs
```

### Relations

```markdown
- implements [[Target Note]]
- part_of [[Larger Concept]]
- relates_to [[Related Topic]] (optional context)
...
memory://path/relation_type/*       # Follow specific relation type
memory://path/*/target              # Find entities with relations to target
memory://project/*/requirements     # Wildcards in paths
```
...
| [question] | Open questions or uncertainties |
| [idea] | Creative concepts or proposals |
| [requirement] | Necessary conditions or features |
| [technique] | Methods or approaches |
| [limitation] | Constraints or boundaries |
| [preference] | Subjective choices |
| [issue] | Problems or challenges |
| [solution] | Resolutions to problems |

## Common Relation Types

| Relation Type | Meaning | Reciprocal |
|---------------|---------|------------|
| implements | Puts into practice | implemented_by |
| part_of | Component relationship | contains |
| relates_to | General connection | relates_to |
| extends | Builds upon | extended_by |
| depends_on | Requires for function | required_for |
| precedes | Comes before | follows |
| inspires | Provides creative basis | inspired_by |
| resolves | Addresses issue | resolved_by |
| contrasts_with | Shows differences | contrasts_with |
...
# Group operators
(python OR javascript) AND web
```

### Combining Search with Context

```
# Find and then follow relations
1. search_notes("authentication")
2. build_context("memory://results[0].permalink", depth=2)
```

## Obsidian Integration

SkogAI-Memory files can be opened in Obsidian for visualization:
...
Creating visual knowledge maps:

```python
await canvas(
    nodes=[
        {"id": "note1", "label": "Project Requirements"},
        {"id": "note2", "label": "Technical Specification"}
    ],
    edges=[
        {"from": "note1", "to": "note2", "label": "defines"}
    ],
...
## Observations
- [category] Observation 1 #tag
- [category] Observation 2 #tag
...
1. **Changes aren't appearing**:
   - Verify `skogcli memory sync --watch` is running
   - Run `skogcli memory status` to check system state
   - Try manual sync with `skogcli memory sync`

2. **Can't find content**:
   - Check exact path and permalink
   - Try searching with more general terms
   - Verify file exists in knowledge base

3. **Relations not working**:
   - Ensure exact title matching in wiki links
   - Check for typos in relation types
   - Verify both documents exist

4. **Error messages**:
   - Always preserve exact error messages
   - Report errors clearly without attempting workarounds
   - Check system status with `skogcli memory status`

---
---
---

Above is the knowledge guide for skogai-memory as well as a memory called skogai-memory-quick-reference.md

Please return your ratings and what you see as good and bad about the memory skogai-memory-quick-reference.md and how it follows the skogai-memory knowledge guide.