...
title: Canvas
type: note
permalink: skogai/memory/canvas
---

# Canvas

Canvas is a powerful visualization tool in SkogAI-Memory that allows you to create graphical representations of your knowledge graph. This document explains how to use canvas for visualizing concepts, relations, and knowledge structures.

## Overview

Canvas creates Obsidian-compatible canvas files that display entities and their relationships in a visual format. These visualizations help you:

- Understand complex knowledge structures
- Identify gaps and connections in your knowledge
- Communicate relationships between concepts
- Plan and organize knowledge hierarchies

## Creating Canvas Visualizations

### Through Conversations

Ask your AI assistant to create a visualization:

```
You: Could you create a canvas visualization of our authentication system components?
```

The assistant will:
1. Gather relevant entities from your knowledge base
2. Determine relationships between them
3. Create a canvas file with appropriate nodes and connections

### Programmatic Creation

Canvas visualizations can be created through the API:

```python
canvas_result = await canvas(
    nodes=[
        {"id": "auth", "label": "Authentication System", "color": "blue"},
        {"id": "jwt", "label": "JWT Implementation", "color": "green"},
        {"id": "user", "label": "User Management", "color": "red"}
    ],
    edges=[
        {"from": "auth", "to": "jwt", "label": "implements"},
        {"from": "auth", "to": "user", "label": "part_of"}
    ],
...
Canvas files are stored as `.canvas` files in your knowledge base. They contain JSON representing:

- **Nodes**: Entities in your knowledge graph
...
Canvas supports different types of nodes:

- **Text Nodes**: Display text information
- **File Nodes**: Link to existing knowledge files
- **Link Nodes**: Connect to external resources
- **Image Nodes**: Display images from your knowledge base

## Customization Options

When creating canvas visualizations, you can customize:

- **Colors**: Assign different colors to categorize nodes
- **Sizes**: Adjust node sizes to indicate importance
- **Labels**: Customize edge labels to show relation types
- **Layout**: Specify hierarchical, radial, or custom layouts
- **Groups**: Create visual groupings of related nodes

## Using Canvas for Different Purposes
...
```
You: Create a canvas of our application architecture showing services and data flows.
```
...
```
You: Make a concept map for machine learning algorithms and their relationships.
```

### Project Planning

Visualize project components, dependencies, and timelines:

```
You: Create a canvas showing our project phases and dependencies.
```

### Decision Trees

Map decision points and their consequences:

```
You: Build a canvas showing the decision tree for our authentication approach.
```

## Viewing Canvas Files

Canvas files can be viewed in:

1. **Obsidian**: Open the `.canvas` file in Obsidian for interactive exploration
2. **Claude/AI**: Ask your AI assistant to describe the contents of a specific canvas file

## Example Canvas Queries

To create effective canvas visualizations:

```
You: Create a canvas of all documents related to authentication in my knowledge base.

You: Build a canvas showing the "Security Architecture" document and all connected notes up to 2 levels deep.

You: Visualize the knowledge graph around "Project Planning" with implementation relationships highlighted.
```

## Best Practices

1. **Focus on Key Relationships**: Include only the most relevant entities and relationships to avoid cluttered visualizations
2. **Use Consistent Colors**: Develop a color coding system (e.g., blue for concepts, green for implementations)
3. **Add Context Labels**: Include descriptive edge labels that explain the nature of relationships
4. **Update Regularly**: Refresh canvas files as your knowledge evolves
5. **Create Multiple Views**: Build different canvases for different perspectives on the same knowledge

## Limitations

- Very large knowledge graphs may become visually cluttered
- Complex relationship types may be difficult to represent
- Non-Obsidian viewers may have limited interaction with canvas files

## Relations

- relates_to [[SkogAI-Memory User Guide]]
- part_of [[SkogAI-Memory Documentation]]
- implements [[Knowledge Visualization]]

---
---
---

Above is the knowledge guide for skogai-memory as well as a memory called Canvas.md

Please return your ratings and what you see as good and bad about the memory Canvas.md and how it follows the skogai-memory knowledge guide.