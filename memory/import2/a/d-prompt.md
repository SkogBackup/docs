...
---
title: SkogParse Inline Commands
type: note
permalink: skog-ai/tools/skog-parse-inline-commands
---

# SkogParse Inline Commands

The inline command feature in SkogParse allows embedding executable commands directly within text content.
...
Where:
- `command` is the name of the command to execute (without the @ prefix)
- `payload` is the input data passed to the command handler

## Example Usage

```
This is some text with an [@echo:inline command] embedded in it.
```

After processing, it might become:
```
This is some text with an You said: inline command embedded in it.
```

## How Inline Commands Work

1. The parser scans text for the pattern `[@command:payload]`
2. For each match, it:
   - Extracts the command name and payload
   - Looks for a script at `./scripts/{command}.sh`
   - If no script exists, looks for a registered handler function
   - Executes the handler with the payload
   - Replaces the entire pattern with the output

## Implementation Details

- Inline commands are processed after standalone commands
- The parser prevents nested command execution by default
- Commands are processed in order from outside to inside
- JSON payloads are automatically parsed if valid

## Usage in CLI

```bash
skogparse parse "Let me greet you: [@greet:Reader]!"
```

## Simplified API Example

```python
from skogparse import parse
from skogparse.utils import register_handler

# Register a handler
register_handler("greet", lambda name: f"Hello, {name}!")

# Parse text with an inline command
result = parse("Welcome [@greet:World]!")
...
You can implement inline commands as shell scripts:
...
- Inline commands provide a flexible way to add dynamic content to text
- They're particularly useful in agent-based systems or template processing
- By default, nested commands (commands inside other commands) are not processed

---
---
---

Above is the knowledge guide for skogai-memory as well as a memory called d.md

Please return your ratings and what you see as good and bad about the memory d.md and how it follows the skogai-memory knowledge guide.