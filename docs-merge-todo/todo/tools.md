...
1. Prefer BatchTool when making multiple related tool calls
1. Use dispatch_agent for complex file searches
1. Update journal entries with daily progress
1. Store important concepts in knowledge/ directory
1. Use task management for tracking work items
1. Test MCP capabilities before relying on them in workflows

## observations

- [fact] Claude has both native tools and MCP-enabled tools in the SkogAI ecosystem #tools #integration
- [capability] MCP memory system provides persistent storage across sessions with semantic search #memory #persistence
- [technique] BatchTool allows parallel execution of multiple tools for performance optimization #performance #efficiency
- [status] GitHub integration and project context tools have known issues requiring attention #testing #issues
- [best-practice] Dispatch agent should be used for complex multi-file searches #search #navigation
- [structure] Common locations organized by purpose (tasks, journal, knowledge, people) #organization #file-structure

## relations

- part_of \[[skogai-ecosystem]\] (provides tooling infrastructure for the ecosystem)
- enables \[[claude-workflows]\] (provides capabilities for Claude's operations)
- integrates_with \[[github-integration]\] (connects to GitHub through MCP)
- supports \[[memory-persistence]\] (enables cross-session information storage)
- implements \[[mcp-protocol]\] (uses Model Context Protocol for extended functionality)
