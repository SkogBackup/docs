---
title: WO-05-docs-decouple-cora
type: note
permalink: skogai/work-orders/wo-05-docs-decouple-cora
---
...
1. Step 2 "Gather Context" (line 68): "Module name: Which CORA module had the problem"
1. Step 2 (line 76): "Rails version" as required environment detail
1. Step 2 (line 77): "Stage (0-6 or post-implementation)" -- CORA-specific staging system
1. Step 7 (line 256): references `docs/solutions/patterns/cora-critical-patterns.md`
1. Decision Menu Option 2 (lines 293-306): references `cora-critical-patterns.md` multiple times
1. Error Handling (line 412): "Module not in CORA-MODULES.md"
1. Example Scenario (lines 463-512): uses CORA-specific modules (Brief System, EmailProcessing)

**CORA-specific references in schema.yaml:**

1. Line 1: `# CORA Documentation Schema`
1. Line 7: `description: "Module/area of CORA"`
1. Component enum (lines 39-57): 17 values, most are CORA-specific:
...
1. Line 57: `description: "CORA component involved"`
1. Line 136: `"module must be a valid CORA module name"`
1. Examples (lines 146-176): use CORA modules and Rails versions

**CORA-specific references in yaml-schema.md:**

1. Line 7: `"Module name (e.g., 'EmailProcessing') or 'CORA' for system-wide issues"`
1. Component enum (line 10): same CORA-specific values as schema.yaml
1. Example (lines 32-46): uses "Email Processing" module, Rails version
